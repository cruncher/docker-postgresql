...
```bash
docker pull cruncher/postgresql:11.5-wale
```
...
* On the master, move the existing DB out of the way: `mv /var/db/11 /var/db/11-old`
...
```bash
docker run --name postgresql -itd --restart always \
  cruncher/postgresql:11.5-wale -c log_connections=on
```
...
```bash
docker run --name postgresql -itd --restart always \
  cruncher/postgresql:11.5-wale -c logging_collector=on
```
