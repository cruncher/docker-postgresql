...
```bash
docker pull cruncher/postgresql:11.6-wale
```
...
```bash
docker run --name postgresql -itd --restart always \
  cruncher/postgresql:11.6-wale -c log_connections=on
```
...
```bash
docker run --name postgresql -itd --restart always \
  cruncher/postgresql:11.6-wale -c logging_collector=on
```
